...
Scorecard variants evolve with the retrieval work. Legacy retrieval scorecards compare Full hybrid against Graph disabled and Vector only. Current scorecards identify variants with stable machine-readable IDs even when display labels change. Newer rerank scorecards compare Full hybrid (rerank off) against Full hybrid + rerank-2. The comparison matters because a real memory system cannot hide behind one strategy; active lexical, tag, alias, vector, and rerank changes must stay inspectable rather than disappearing behind a single headline.
